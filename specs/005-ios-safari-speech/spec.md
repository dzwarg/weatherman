...
A child on an iPhone or iPad opens the app and says "good morning weatherbot." The app hears the wake phrase and transitions to active listening — every time, not just the first time. Today, the app silently stops listening after the first recovery attempt, leaving children speaking to an unresponsive app.
...
1. **Given** the app is open on an iPhone running iOS 17, **When** the child says "good morning weatherbot," **Then** the app enters active listening mode and responds with a spoken greeting.
2. **Given** the app has just completed a voice interaction on an iPad (iPadOS 16+), **When** the child says the wake phrase again after the response finishes, **Then** the app detects it and starts a new interaction.
...
- If the child speaks the wake phrase while the app is still delivering a spoken response, the wake phrase is ignored; the response plays to completion and wake word detection resumes automatically afterward.
...
- **FR-006**: The app MUST display a clearly visible, user-friendly message when the current browser does not support voice recognition, rather than silently disabling the feature.
...
- **FR-008**: The app MUST support wake word detection that recovers from at least 3 consecutive operating-system-initiated session endings without requiring a page reload.
- **FR-009**: While a spoken response is playing, the app MUST ignore any wake phrase detections and resume wake word detection automatically once the response finishes.
...
- **Wake Word**: The specific phrase ("good morning weatherbot") that transitions the app from passive listening to active query mode.
...
- **SC-006**: Users on unsupported browsers see a clear explanation within 2 seconds of the page loading, rather than a blank or broken voice interface.
...
- The existing wake phrase ("good morning weatherbot") and response flow are not changing — only the reliability of the voice pipeline on Safari/iOS is in scope.
